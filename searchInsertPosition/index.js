...
var searchInsert = function (nums, target) {
  ...
  if (index === -1) { // not in array, then find position where to insert using binary search
    ...
    function binarySearch(low, mid, high, n) {
      ...
      if (target < nums[mid]) {
        binarySearch(low, Math.floor(mid / 2), high = mid, n = mid);
      } else {
        low = mid;
        mid = low + Math.floor((high-low) / 2); // 4-2 = 2 => high-low + Math.floor(2/2)
        binarySearch(low, mid, high, n = high - low-1);
      }
    }
    binarySearch(low, mid, high, n); // pass indices
